...
from sklearn import model_selection, preprocessing, linear_model, svm, decomposition, tree, ensemble
...
#def algorithms(questions_train,rev_train,labels_train,len1,questions_test,rev_test,labels_test,len2,timestr):
...
accuracy,cm, predictions = train_model("Logistic Regression on Count Vectors",linear_model.LogisticRegression(C = 1), counts_train, labels_train, counts_test, labels_test)
print ("LR, Count Vectors: ", accuracy)


df_buffer = pd.DataFrame({'Question': questions_test,'Answer': rev_test,'Actual': labels_test, 'Predicted': predictions})
df_buffer.to_csv("final_op/LR_Count_Vectors.csv")

df_lr1 = pd.DataFrame([cm],columns = col)
df_algorithm_analysis = df_algorithm_analysis.append(df_lr1)




# Linear Classifier on Word Level TF IDF Vectors
accuracy,cm, predictions = train_model("Logistic Regression on Word Level TF IDF",linear_model.LogisticRegression(C = 1),  xtrain_tfidf, labels_train, xvalid_tfidf, labels_test)
print ("LR, WordLevel TF-IDF: ", accuracy)

df_buffer = pd.DataFrame({'Question': questions_test,'Answer': rev_test,'Actual': labels_test, 'Predicted': predictions})
df_buffer.to_csv("final_op/LR_Word_lvl_tfidf.csv")

df_lr2 = pd.DataFrame([cm],columns = col)
df_algorithm_analysis = df_algorithm_analysis.append(df_lr2)


# Linear Classifier on Ngram Level TF IDF Vectors
accuracy,cm, predictions = train_model("Logistic Regression on Ngram Level TF IDF", linear_model.LogisticRegression(C = 1), xtrain_tfidf_ngram, labels_train, xvalid_tfidf_ngram, labels_test)
print ("LR, N-Gram Vectors: ", accuracy)

df_buffer = pd.DataFrame({'Question': questions_test,'Answer': rev_test,'Actual': labels_test, 'Predicted': predictions})
df_buffer.to_csv("final_op/LR_Ngram_lvl_tfidf.csv")

df_lr3 = pd.DataFrame([cm],columns = col)
df_algorithm_analysis = df_algorithm_analysis.append(df_lr3)



# Linear Classifier on Character Level TF IDF Vectors
accuracy,cm, predictions = train_model("Logistic Regression on Character Level TF IDF", linear_model.LogisticRegression(C = 1), xtrain_tfidf_ngram_chars, labels_train, xvalid_tfidf_ngram_chars, labels_test)
print ("LR, CharLevel Vectors: ", accuracy)

df_buffer = pd.DataFrame({'Question': questions_test,'Answer': rev_test,'Actual': labels_test, 'Predicted': predictions})
df_buffer.to_csv("final_op/LR_Char_lvl_tfidf.csv")

df_lr4 = pd.DataFrame([cm],columns = col)
df_algorithm_analysis = df_algorithm_analysis.append(df_lr4)
